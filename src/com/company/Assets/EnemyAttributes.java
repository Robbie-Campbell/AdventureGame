...
public class EnemyAttributes {
    ...
    public EnemyAttributes(int health){
        this.health = health;
        String[] name = {"Barry", "Gary", "Larry", "Geoff", "Steve", "Carl"};
        nameChoice = name[rand.nextInt(6)];
    }
    ...
    private void attack(){
        ...
        if (this.health > 0) {
            if (successfulHit > 0 && successfulHit < 10) {
                ...
                System.out.printf("%s attacked for a critical!! Your new Health is %d/%d\n", this.nameChoice, player.health, player.maxHealth);
            } else if (successfulHit < 60) {
                player.health -= this.attackDamage;
                System.out.printf("%s attacked successfully! Your new Health is %d/%d\n", this.nameChoice, player.health, player.maxHealth);
            } else if (successfulHit < 90) {
                System.out.printf("%s Missed his attack!\n", this.nameChoice);
            } else {
                System.out.printf("%s Missed so badly he killed himself!\n", this.nameChoice);
                this.health = 0;
            }
        }
    }
